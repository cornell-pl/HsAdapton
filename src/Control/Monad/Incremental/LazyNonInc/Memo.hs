...
instance WeakRef r => Memo (LazyNonIncM l inc r m a) where
	...
	{-# INLINE memoKey #-}
	memoKey x@(LazyNonIncM r) = (MkWeak $ WeakRef.mkWeakWithRefKey r,unsafePerformIO $ makeStableName x)
                                 
instance WeakRef r => Memo (LazyNonIncU l inc r m a) where
	type Key (LazyNonIncU l inc r m a) = StableName (LazyNonIncU l inc r m a)
	{-# INLINE memoKey #-}
	memoKey x@(LazyNonIncU r) = (MkWeak $ WeakRef.mkWeakWithRefKey r,unsafePerformIO $ makeStableName x)

instance WeakRef r => Memo (LazyNonIncL l inc r m a) where
	type Key (LazyNonIncL l inc r m a) = StableName (LazyNonIncL l inc r m a)
	{-# INLINE memoKey #-}
	memoKey x@(LazyNonIncL r) = (MkWeak $ WeakRef.mkWeakWithRefKey r,unsafePerformIO $ makeStableName x)
