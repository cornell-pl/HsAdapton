{-# LANGUAGE StandaloneDeriving, DeriveDataTypeable, TypeFamilies, TypeOperators, TemplateHaskell, RankNTypes, ScopedTypeVariables, FlexibleContexts, KindSignatures #-}
...
everythingInc :: (Typeable ctx,Typeable b,Eq b,Output thunk l inc r m,MonadIO m) => Proxy ctx -> (thunk l inc r m b -> l inc r m b -> l inc r m b) -> GenericQMemo ctx thunk l inc r m b -> GenericQMemo ctx thunk l inc r m b
...
everythingButInc :: (Typeable ctx,Typeable b,Eq b,Output thunk l inc r m,MonadIO m) => Proxy ctx -> (thunk l inc r m b -> l inc r m b -> l inc r m b) -> GenericQMemoBut ctx thunk l inc r m b -> GenericQMemo ctx thunk l inc r m b
...
mkQInc :: (Typeable c,Eq c,Output thunk l inc r m,Typeable a,Typeable b,Monad m) => l inc r m c -> (b -> l inc r m c) -> a -> l inc r m (thunk l inc r m c)
...
mkQButTypeInc :: (Typeable c,Eq c,Output thunk l inc r m,DeepTypeable a,DeepTypeable b,Monad m) => l inc r m c -> (b -> l inc r m c) -> a -> l inc r m (thunk l inc r m c,Bool)
...
gmapQrInc :: (Typeable b,Eq b',MData (MemoCtx ctx) (l inc r m) a,Output thunk l inc r m,Eq b) => Proxy ctx -> (thunk l inc r m b' -> l inc r m b -> l inc r m b) -> l inc r m b -> GenericQMemo ctx thunk l inc r m b' -> a -> l inc r m (thunk l inc r m b)
...
listifyInc :: (Typeable ctx,Eq b,Eq (JoinListMod thunk l inc r m b),DeepTypeable b,Output thunk l inc r m,MonadIO m) => Proxy ctx -> (b -> l inc r m Bool) -> GenericQMemo ctx thunk l inc r m (JoinListMod' thunk l inc r m b)
