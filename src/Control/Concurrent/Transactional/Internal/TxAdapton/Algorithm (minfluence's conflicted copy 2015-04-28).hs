...
instance (TxLayer Outside c,TxLayer Inside c) => Thunk (TxM i c) Inside (TxAdapton c) where
	new = modInnerTxM
	{-# INLINE new #-}
	newc = refInnerTxM
	{-# INLINE newc #-}
...
instance (TxLayer Outside c) => Thunk (TxM i c) Outside (TxAdapton c) where
	new = modOuterTxM
	{-# INLINE new #-}
	newc = refOuterTxM
	{-# INLINE newc #-}
...
instance (TxLayer Outside c,TxLayer Inside c) => Input (TxM i c) Inside (TxAdapton c) where
	ref = refInnerTxM
	{-# INLINE ref #-}
	get = getInnerTxM
	{-# INLINE get #-}
...
instance (TxLayer Outside c) => Input (TxM i c) Outside (TxAdapton c) where
	ref = refOuterTxM
	{-# INLINE ref #-}
	get = getOuterTxM
	{-# INLINE get #-}
...
instance TxLayer Outside c => Isolated (TxM i c) Outside (TxAdapton c) where
	isolated = modOuterTxMWith
instance (TxLayer Inside c,TxLayer Outside c) => Isolated (TxM i c) Inside (TxAdapton c) where
	isolated = modInnerTxMWith

modInnerTxMWith :: (TxLayer Outside c,IncK (TxAdapton c) a,TxLayer Inside c) => Resolve (TxAdapton c) a -> Inside (TxAdapton c) a -> Inside (TxAdapton c) (TxM i c Inside (TxAdapton c) a)
modInnerTxMWith resolve m = m >>= refInnerTxMWith resolve
modOuterTxMWith :: (IncK (TxAdapton c) a,TxLayer Outside c) => Resolve (TxAdapton c) a -> Outside (TxAdapton c) a -> Outside (TxAdapton c) (TxM i c Outside (TxAdapton c) a)
modOuterTxMWith resolve m = m >>= refOuterTxMWith resolve

modInnerTxM :: (TxLayer Outside c,IncK (TxAdapton c) a,TxLayer Inside c) => Inside (TxAdapton c) a -> Inside (TxAdapton c) (TxM i c Inside (TxAdapton c) a)
modInnerTxM m = m >>= refInnerTxM
modOuterTxM :: (IncK (TxAdapton c) a,TxLayer Outside c) => Outside (TxAdapton c) a -> Outside (TxAdapton c) (TxM i c Outside (TxAdapton c) a)
modOuterTxM m = m >>= refOuterTxM

refOuterTxM :: (IncK (TxAdapton c) a,TxLayer l c,TxLayer Outside c) => a -> Outside (TxAdapton c) (TxM i c l (TxAdapton c) a)
refOuterTxM = refOuterTxMWith versioned

refOuterTxMWith :: (IncK (TxAdapton c) a,TxLayer l c,TxLayer Outside c) => Resolve (TxAdapton c) a -> a -> Outside (TxAdapton c) (TxM i c l (TxAdapton c) a)
refOuterTxMWith (resolve :: Resolve (TxAdapton c) a) v = doBlockTx $ do
	...
	m <- unsafeIOToInc $! do
		...
		let meta = TxNodeMeta
			...
			:!: content
			:!: unmemo :!: drawTxM m)
		    m = TxM (dta :!: meta :!: resolve)
		return m
	newTxMLog m
	return $! m

refInnerTxM :: (IncK (TxAdapton c) a,TxLayer l c,TxLayer Outside c,TxLayer Inside c) => a -> Inside (TxAdapton c) (TxM i c l (TxAdapton c) a)
refInnerTxM = refInnerTxMWith versioned

refInnerTxMWith :: (TxLayer Outside c,IncK (TxAdapton c) a,TxLayer l c,TxLayer Inside c) => Resolve (TxAdapton c) a -> a -> Inside (TxAdapton c) (TxM i c l (TxAdapton c) a)
refInnerTxMWith (resolve :: Resolve (TxAdapton c) a) v = doBlockTx $ do
	let proxy = Proxy :: Proxy c
...
{-# INLINE getInnerTxM #-}
getInnerTxM :: (TxLayer Outside c,IncK (TxAdapton c) a,TxLayer Inside c) => TxM i c Inside (TxAdapton c) a -> Inside (TxAdapton c) a
...
{-# INLINE getOuterTxM #-}	
getOuterTxM :: (IncK (TxAdapton c) a,TxLayer l c,TxLayer Outside c) => TxM i c l (TxAdapton c) a -> Outside (TxAdapton c) a
getOuterTxM = \t -> {-# SCC getOuterTxM #-} liftM Prelude.fst $ readTxMValue t

{-# INLINE checkTxM #-}
checkTxM :: (TxLayer Outside c,IncK (TxAdapton c) a,TxLayer Inside c) => TxM i c Inside (TxAdapton c) a -> a -> Inside (TxAdapton c) (Bool,TxStatus)
checkTxM t oldv = do
	(!value,!status) <- readTxMValue t
	let !ok = oldv == value
	return $! (ok,status)

setInnerTxM :: (IncK (TxAdapton c) a,TxLayer Outside c,TxLayer Inside c) => TxM i c Inside (TxAdapton c) a -> a -> Outside (TxAdapton c) ()
...
setOuterTxM :: (IncK (TxAdapton c) a,TxLayer Outside c) => TxM i c Outside (TxAdapton c) a -> a -> Outside (TxAdapton c) ()
...
drawTxM :: (TxLayer Outside c,IncK (TxAdapton c) a,TxLayer l c) => TxM i c l (TxAdapton c) a -> Outside (TxAdapton c) DrawDot
...
waitForChildrenTx :: TxLayer l c => l (TxAdapton c) ()
waitForChildrenTx = do
  ...
  where
	waitForChild :: DynTxFuture c -> IO ()
	waitForChild (DynTxFuture (TxFuture result)) = do
		e <- readMVar "waitForChildrenTx" result
		case e of
			Left ex -> Control.Exception.throw ex
			Right v -> return ()
...
atomicallyTx (params :: IncParams (TxAdapton c)) msg (stm :: l (TxAdapton c) a) = initializeTx params try where
	...
	catchRetry BlockedOnRetry = flip Catch.catches [unmaskedHandler catchInvalid] $ unTxLayer l $ do -- concurrent invalidation exceptions
		killChildrenTx
		...
		case mbsuccess of
			Left lck -> do -- retried txs are always in a consistent state, because we apply all affecting updates before releasing the lock
				...
				debugTx $ "woke up tx"
				starttime <- readTxId
				restartTxWithRepair Nothing ("try: BlockedOnRetry retrying invalid tx previously known as " ++ show starttime) starttime try
			Right repair -> do
				starttime <- readTxId
				let withRepair = if (txAdaptonRepair params) then Just repair else Nothing
...
retryTx = unsafeIOToInc $ throwIO BlockedOnRetry
...
orElseTx (stm1 :: Outside (TxAdapton c) a) stm2 = do1 where
	...
	catchRetry1 BlockedOnRetry = unTxLayer proxyOutside $ validateAndRetryNestedTx "orElseRetry1" >> do2
	catchRetry2 BlockedOnRetry = unTxLayer proxyOutside (validateAndRetryNestedTx "orElseRetry2") >> Catch.throwM BlockedOnRetry
	catchInvalid (e::InvalidTx c) = Catch.throwM e
...
futureTx stm = do
  ...
  where
	try result = do
		...
		case mb of
			Nothing -> unsafeIOToInc $ putMVar result $ Right x
			Just repair -> unsafeIOToInc $ putMVar result $ Left $ toException $ InvalidTx repair

joinTx :: (TxLayer Inside c,TxLayer Outside c) => TxFuture c a -> Outside (TxAdapton c) a
joinTx f = do
	e <- readMVar f
	case e of
		Left ex -> do
			commitFutureTx False
		Right v -> do
			commitFutureTx True
...
forkNestedTx rootThread (f :: TxFuture c a) m = do
	...
	unsafeIOToInc $ do
		mkSnap <- snapshotTxLogs (txAdaptonMemoSize params) rootThread txlogs
		forkIO $ do
			thread <- myThreadId
			snap <- mkSnap thread
		...
		return ()
  where
	catchRetry BlockedOnRetry = unTxLayer proxyOutside (validateFutureTx "futureRetry" Nothing) >> lift (exceptionalFutureTx rootThread f BlockedOnRetry)
	catchInvalid (e::InvalidTx c) = lift (exceptionalFutureTx rootThread f e)
	catchSome (e::SomeException) = do
		case fromException e :: Maybe AsyncException of
			Just ThreadKilled -> return () -- die gracefully if killed by parent
			otherwise -> do
				unTxLayer proxyOutside (validateFutureTx "futureSome" (Just e)) >> lift (exceptionalFutureTx rootThread f e)
...
commitNestedTx isFuture rootThread thread doWrites txlog_child txlogs_parent = {-# SCC flattenTxLogs #-} do
	...
	if doWrites
		then mergeTxLog isFuture thread txlog_child txlogs_parent
		else extendTxLog isFuture thread txlog_child txlogs_parent -- we don't need to discard @Evals@
	-- merges the buffered memo table entries for a txlog with its parent
	mergeTxLogMemos rootThread thread txlog_child txlogs_parent
	return ()
...
commitFutureTx :: (TxLayer Inside c,TxLayer Outside c) => String -> Bool -> Outside (TxAdapton c) ()
commitFutureTx msg doWrites = do
...
instance TxConflictClass EarlyConflict where
	...
	exceptionalFutureTx rootThread (TxFuture result) e = throwTo rootThread e
...
instance TxConflictClass CommitConflict where
	...
	exceptionalFutureTx rootThread (TxFuture result) e = putMVar result (Left $ toException e)
...
instance (TxLayer Inside c,TxLayer Outside c) => Promising (Outside (TxAdapton c)) where
	...
	type Promise (Outside (TxAdapton c)) = STxMVar (TxM i c) (TxAdapton c)
	type PromiseK (Outside (TxAdapton c)) a = STxMVarK (TxM i c) (TxAdapton c) a
...
instance (TxLayer Inside c,TxLayer Outside c) => Futuristic (Outside (TxAdapton c)) where
	...
	type FutureK (Outside (TxAdapton c)) a = STxMVarK (TxM i c) (TxAdapton c) a
