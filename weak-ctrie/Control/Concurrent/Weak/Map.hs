...
lookupOrInsert w_tbl@(WeakMap (tbl_ref :!: weak_tbl)) k mv mkWeak = do
	...
	CMap.lookupOrInsert k Weak.deRefWeak (mv >>= \l -> liftM (,l) $ mkWeak l $ Just $ deleteFinalized' weak_tbl k) (Weak.finalize . Prelude.fst) tbl
...
lookupOrInsertMkWeak w_tbl@(WeakMap (tbl_ref :!: weak_tbl)) k mv mk = do
	tbl <- readIORef tbl_ref
	CMap.lookupOrInsert k Weak.deRefWeak (mv >>= \l -> liftM (,l) $ let (MkWeak mkWeak) = mk l in mkWeak l $ Just $ deleteFinalized' weak_tbl k) (Weak.finalize . Prelude.fst) tbl
...
empty = do
	...
	weak_tbl <- Weak.mkWeakKey tbl_ref tbl $ Just $ table_finalizer tbl
