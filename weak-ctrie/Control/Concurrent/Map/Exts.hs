...
-- If the key is already present in the map, the new value is not written and the old value is retrieved.
-- The predicate decides to overlap the existing entry or not
lookupOrInsert :: (Eq k, Hashable k) => k -> (v -> IO (Maybe r)) -> IO (v,r) -> ((v,r) -> IO ()) -> Map k v -> IO r
lookupOrInsert k p mv purge (Map root) = go0
    where
        h = hash k
            ...
            case peekTicket ticket of
                    ...
                    if bmp .&. m == 0
                        then do
                            ...
                            let arr' = A.insert (SNode (S k v)) i n arr
                                cn'  = CNode (bmp .|. m) arr'
                            b <- (fst <$> casIORef inode ticket cn')
                            if b then return r else purge p >> go0
                            ...
                            SNode (S k2 v2)
                                ...
                                | otherwise -> do
                                    p@(v,r) <- mv                            
                                    let h2 = hash k2
                                    inode2 <- oldINode h k v h2 k2 v2 (nextLevel lev)
                ...
                Collision arr -> do
                	let go1 = do
                		...
                		let col' = Collision arr'
                		b <- (fst <$> casIORef inode ticket col')
                		if b then return r else purge p >> go0
                	case List.find (\(S k2 _) -> k2 == k) arr of
                		Just (S _ v2) -> do
							mb <- p v2
							case mb of
								Just r -> return r
								Nothing -> go1
                		_            -> go1
{-# INLINABLE lookupOrInsert #-}

oldINode :: Hash -> k -> v -> Hash -> k -> v -> Int -> IO (INode k v)
oldINode h1 k1 v1 h2 k2 v2 lev
    | lev >= hashLength = newIORef $ Collision [S k2 v2]
    | otherwise = do
        let i1 = index h1 lev
            i2 = index h2 lev
